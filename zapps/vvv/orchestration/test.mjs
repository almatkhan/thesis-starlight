...
import unVault from "./unVault.mjs";
...
describe("ZKSenderShield", async function () {
		...
		describe("First call", async function () {
			...
			it("should call vault", async () => {
				try {
					...
					const { tx, encEvent, encBackupEvent } = await vault(166);
					// prints the tx
					console.log(tx);
			...
			it("should update the merkle tree", async () => {
				try {
					...
					const path = await getSiblingPath("ZKSenderShield", leafIndex);
	...
	// eslint-disable-next-line func-names
	describe("unVault", async function () {
		this.timeout(3660000);
		try {
			await web3.connect();
		} catch (err) {
		...
		describe("First call", async function () {
			this.timeout(3660000);
			it("should call unVault", async () => {
				try {
					// this starts up the merkle tree's event filter
					await startEventFilter("ZKSenderShield");
					// this calls your function! It returns the tx from the shield contract
					// you can replace the values below - numbers are randomly generated
					const { tx, encEvent, encBackupEvent } = await unVault(27);
					// prints the tx
					console.log(tx);
					// reassigns leafIndex to the index of the first commitment added by this function
					if (tx.event) {
