...
export class SendManager {
	...
	async send(
		...
		let balances_recipient_stateVarIdInit = 11;

		const balances_recipient_stateVarId_key = recipient;

		let balances_recipient_stateVarId = generalise(
			utils.mimcHash(
				[
					generalise(balances_recipient_stateVarIdInit).bigInt,
		...
		const allInputs = [
			...
			balances_recipient_newOwnerPublicKey.integer,
			balances_recipient_newSalt.integer,
			balances_recipient_newCommitment.integer,
		].flat(Infinity);
		const res = await generateProof("send", allInputs);
		...
		// Encrypt pre-image for state variable balances_recipient as a backup:

		let balances_recipient_ephSecretKey = generalise(utils.randomHex(31));

		let balances_recipient_ephPublicKeyPoint = generalise(
			scalarMult(
				balances_recipient_ephSecretKey.hex(32),
				config.BABYJUBJUB.GENERATOR
			)
		);

		let balances_recipient_ephPublicKey = compressStarlightKey(
			balances_recipient_ephPublicKeyPoint
		);

		while (balances_recipient_ephPublicKey === null) {
			balances_recipient_ephSecretKey = generalise(utils.randomHex(31));

			balances_recipient_ephPublicKeyPoint = generalise(
				scalarMult(
					balances_recipient_ephSecretKey.hex(32),
					config.BABYJUBJUB.GENERATOR
				)
			...
			balances_recipient_ephPublicKey = compressStarlightKey(
				balances_recipient_ephPublicKeyPoint
			);
		}

		const balances_recipient_bcipherText = encrypt(
			[
				BigInt(balances_recipient_newSalt.hex(32)),
				BigInt(recipient.hex(32)),
				BigInt(generalise(balances_recipient_stateVarIdInit).hex(32)),
				BigInt(balances_recipient_newCommitmentValue.hex(32)),
			],
			balances_recipient_ephSecretKey.hex(32),
			[
				decompressStarlightKey(balances_recipient_newOwnerPublicKey)[0].hex(32),
				decompressStarlightKey(balances_recipient_newOwnerPublicKey)[1].hex(32),
			]
		);

		let balances_recipient_cipherText_combined = {
			varName: "balances a u",
			cipherText: balances_recipient_bcipherText,
			ephPublicKey: balances_recipient_ephPublicKey.hex(32),
		};

		BackupData.push(balances_recipient_cipherText_combined);
