...
import { UnVaultManager } from "./unVault.mjs";
...
export class ServiceManager {
	constructor(web3) {
		...
		this.unVault = new UnVaultManager(web3);
	}
	async init() {
		await this.vault.init();
		await this.send.init();
		await this.unVault.init();
	}
	...
	// eslint-disable-next-line func-names
	async service_unVault(req, res, next) {
		try {
			await startEventFilter("ZKSenderShield");
			const { amountOut } = req.body;
			const balances_msgSender_newOwnerPublicKey =
				req.body.balances_msgSender_newOwnerPublicKey || 0;
			const { tx, encEvent, encBackupEvent } = await this.unVault.unVault(
				amountOut,
				balances_msgSender_newOwnerPublicKey
			);
			// prints the tx
