...
export class VaultManager {
	...
	async vault(_amountIn, _balances_msgSender_newOwnerPublicKey = 0) {
		const instance = this.instance;
		const contractAddr = this.contractAddr;
		const web3 = this.web3;

		const msgValue = 0;
		const amountIn = generalise(_amountIn);
		...
		const txData = await instance.methods
			.vault(
				amountIn.integer,
				{
					...
					newNullifiers: [],
					commitmentRoot: 0,
					newCommitments: [balances_msgSender_newCommitment.integer],
					cipherText: [],
					encKeys: [],
				},
				proof,
