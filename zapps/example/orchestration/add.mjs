...
export class AddManager {
	...
	async add(
		...
		const a_bcipherText = encrypt(
			[BigInt(a_newSalt.hex(32)), BigInt(a_stateVarId), BigInt(a.hex(32))],
		...
		// Encrypt pre-image for state variable c as a backup:

		let c_ephSecretKey = generalise(utils.randomHex(31));

		let c_ephPublicKeyPoint = generalise(
			scalarMult(c_ephSecretKey.hex(32), config.BABYJUBJUB.GENERATOR)
		);

		let c_ephPublicKey = compressStarlightKey(c_ephPublicKeyPoint);

		while (c_ephPublicKey === null) {
			c_ephSecretKey = generalise(utils.randomHex(31));

			c_ephPublicKeyPoint = generalise(
				scalarMult(c_ephSecretKey.hex(32), config.BABYJUBJUB.GENERATOR)
			);

			c_ephPublicKey = compressStarlightKey(c_ephPublicKeyPoint);
		}

		const c_bcipherText = encrypt(
			[BigInt(c_newSalt.hex(32)), BigInt(c_stateVarId), BigInt(c.hex(32))],
			c_ephSecretKey.hex(32),
			[
				decompressStarlightKey(c_newOwnerPublicKey)[0].hex(32),
				decompressStarlightKey(c_newOwnerPublicKey)[1].hex(32),
			]
		);

		let c_cipherText_combined = {
			varName: "c",
			cipherText: c_bcipherText,
			ephPublicKey: c_ephPublicKey.hex(32),
		};

		BackupData.push(c_cipherText_combined);

		// Send transaction to the blockchain:

		const txData = await instance.methods
			.add(
				parseInt(value_publicbool.integer, 10),
				{
					customInputs: [1],
					nullifierRoot: a_nullifierRoot.integer,
					latestNullifierRoot: a_newNullifierRoot.integer,
					newNullifiers: [a_nullifier.integer, c_nullifier.integer],
					commitmentRoot: a_root.integer,
					newCommitments: [a_newCommitment.integer, c_newCommitment.integer],
					cipherText: [],
					encKeys: [],
				},
				proof,
